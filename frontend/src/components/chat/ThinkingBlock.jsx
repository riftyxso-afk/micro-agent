...
export const ThinkingBlock = ({ state, open, onToggle, steps, reasoningText }) => {
  ...
  return (
    <div className="mb-4 rounded-2xl border border-[#E5E7EB] bg-white px-4 py-3 shadow-[0_10px_30px_rgba(17,24,39,0.04)]" data-testid="thinking-block">
      ...
      >
        <span>{hasReasoning ? "Reasoning" : "Thinking"}</span>
        {(state === "thinking" || state === "pending") && (
      ...
      <div className={`ma-collapse ${open ? "ma-collapse-open" : ""}`} aria-hidden={!open}>
        <div className="overflow-hidden">
          <div data-testid="thinking-content" className="mt-2 space-y-2 border-l border-[#E5E7EB] pl-4">
            {hasReasoning && (
              <p className="text-[13px] leading-relaxed whitespace-pre-wrap text-[#6B7280]">{reasoningText}</p>
            )}
            {hasSteps && steps.map((step, i) => {
              ...
              return (
                <p key={`${step}-${i}`} className="text-[13px] leading-relaxed text-[#6B7280]">{step}</p>
              );
            })}
          </div>
        </div>
