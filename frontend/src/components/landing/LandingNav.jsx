  ...
  return (
    <>
      <motion.header
        ...
        className="fixed inset-x-0 top-0 z-50 flex flex-row items-center justify-between border-b border-black/5 bg-white/70 px-5 py-4 backdrop-blur-xl sm:px-8 sm:py-5"
      >
        {/* Logo */}
        <a
          href="/"
        ...
        >
          <Logo size={32} />
          <span className="select-none text-[21px] font-medium tracking-tight text-black sm:text-[24px]">
            MicroAgent
          </span>
        </a>
        ...
        <div className="hidden items-center gap-2.5 md:flex">
          ...
          <button
            ...
            onClick={() => navigate("/home")}
            className="ma-focus rounded-full bg-black px-5 py-2.5 text-[15px] font-medium text-white shadow-[0_4px_14px_rgba(0,0,0,0.18)] transition-all duration-200 ease-out hover:bg-neutral-800 hover:shadow-[0_6px_20px_rgba(0,0,0,0.22)] active:scale-[0.98]"
          >
            Start for free
          </button>
        </div>
        ...
        >
          <span className="relative block h-4 w-5">
            <span
              className={`absolute left-0 top-0 block h-[1.8px] w-5 rounded-full bg-black transition-all duration-300 ease-out ${
                isMobileMenuOpen ? "top-[7px] rotate-45" : ""
              }`}
            />
            <span
              className={`absolute left-0 top-[7px] block h-[1.8px] w-5 rounded-full bg-black transition-all duration-200 ease-out ${
                isMobileMenuOpen ? "opacity-0" : ""
              }`}
            />
            <span
              className={`absolute left-0 top-[14px] block h-[1.8px] w-5 rounded-full bg-black transition-all duration-300 ease-out ${
                isMobileMenuOpen ? "top-[7px] -rotate-45" : ""
              }`}
            />
          </span>
      ...
      <div
        ...
        className={`fixed inset-0 z-[49] flex flex-col bg-white/95 backdrop-blur-sm transition-all duration-300 ease-out md:hidden ${
          isMobileMenuOpen
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-7 px-8">
          {NAV_LINKS.map((link) => (
            <a
              ...
              className="ma-focus rounded-lg text-[26px] font-medium tracking-tight text-black transition-opacity hover:opacity-60"
            >
              {link.label}
            </a>
          ))}
